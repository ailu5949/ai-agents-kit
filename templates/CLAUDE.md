...
3. **读 workflow flags**(决定本项目启用哪些可选阶段):
   ```bash
   jq '.workflow | {design_doc, test_cases, adversarial_review}' .aiagents/config.json
   ```
   - `design_doc.enabled == true` → 本项目启用设计文档,需求拆解阶段必产 `01.5-设计.md`
   - `test_cases.enabled == true` → 本项目启用测试用例,需求拆解阶段必产 `01.6-测试用例.md`
   - `adversarial_review.enabled == true` → 真打验证后多一道对抗审查(`/adversarial-review`,见下方专节)
   - flag 都 false(默认)→ 跳过对应可选阶段
...
| 查 agent 状态 | `/status` |
| 手动触发审查 | `/review` |
| 对抗性审查(异构 provider 找茬,需 flag 开) | `/adversarial-review <agent>` |
| 写一条经验记忆 | `/memory` |
...
**四段式工作流**(对抗审查为可选第三段,`workflow.adversarial_review.enabled` 控制):

```
编码 agent 完成 → state=done-awaiting-review
       ↓ 主 Claude 走 Karpathy 6 项审查(自查:对照你写的 02/03)
state=claude-verifying ← 主 Claude **真打**跑测试 + curl smoke + E2E
       ↓ 全过
   【可选】对抗审查 ← 异构 provider(默认 codex)独立找茬(他查:防同体盲区)
       ↓ PASS
...
4. 真打全过 + `workflow.adversarial_review.enabled == true` → 跑 `/adversarial-review <agent>`(见下方专节);FAIL → 04-Bug修复 → 派回 → 回起点
5. 全过(含对抗审查 PASS,或该 flag 关)→ 写 `state=ready-for-human` + 通知 Lane
...
## ⛔ 对抗性审查 (Adversarial Review) — 可选第三道

**触发条件**: `workflow.adversarial_review.enabled == true`(启动规程已读)。flag 关时整段跳过,真打验证后直接 ready-for-human。

**为什么要它**: Karpathy 6 项 + 真打验证都是**你自查** —— 而你既写 02/03 spec 又审查,如果 spec 理解偏了,你会按(同样偏的)理解判 pass,这是**同体盲区**。对抗审查引入一个**与编码 agent 异构的 provider**(默认 codex,即编码用 claude/opus 时换 codex 这个"另一个脑子")独立找茬。

**何时跑**: 真打验证(Pre-Human Gate 全表)**全绿之后**、推 `state=ready-for-human`**之前**。是最后一道门。
...
**仲裁纪律**:
- ❌ 严禁因"它应该没事"略过 codex 报的问题 —— 要推翻必须在 review 里给反驳证据(读代码/跑测试证明 codex 看错)
- ❌ 严禁自己改业务码修 codex 找到的问题 —— 仍走 04 修复 → 派回编码 agent
- ❌ 严禁 reviewer 异常时默认通过
- ✅ 对抗审查 FAIL 修复后,**重跑真打验证 + 再跑一次对抗审查**(不是只补那一处)
- ✅ 审查报告 § 真打验证段之后增 § 对抗审查段:reviewer / 裁决 / 主要问题 / 你的仲裁结论

**异构性**: reviewer 由 `workflow.adversarial_review.provider` 配置(默认 codex)。若它与编码 agent 的 provider 相同,脚本会警告(异构性丢失) —— 编码 claude 配 reviewer codex,或反之。
