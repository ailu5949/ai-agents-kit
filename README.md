...
### 可选阶段产物(`--with-design-doc` / `--with-test-cases`)

主 Claude 默认只产 `01-需求.md` + `02/03-编码.md`。复杂项目(多模块 / 多状态机 / 跨服务)可启用两个可选阶段:

| Flag | 产物 | 内容 |
|---|---|---|
| `--with-design-doc` | `specs/01.5-设计.md` | 架构图 / 数据模型 / 接口契约 / 状态机 / 关键设计决策 |
| `--with-test-cases` | `specs/01.6-测试用例.md` | 用例表(TC-ID / Given/When/Then)+ 边界条件 + 反向对齐验收点 |
